...
def draw_axis(img, rvecs, tvecs, mtx, dist):
    ...
    oriPoint = tuple(imgpts[0].ravel().astype(np.int))
    x_axis = tuple(imgpts[1].ravel().astype(np.int))
    y_axis = tuple(imgpts[2].ravel().astype(np.int))
    z_axis = tuple(imgpts[3].ravel().astype(np.int))
    
    # print (oriPoint, "   ", x_axis)
